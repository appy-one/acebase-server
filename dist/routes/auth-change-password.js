    ...
    env.app.post(`/auth/${env.db.name}/change_password`, async (req, res) => {
        ...
        try {
            ...
            await env.authRef.child(details.uid).transaction(snap => {
                ...
                let user = snap.val();
                user.uid = snap.key;
                let hash = user.password_salt ? password_1.getPasswordHash(details.password, user.password_salt) : password_1.getOldPasswordHash(details.password);
                ...
                let pwd = password_1.createPasswordHash(details.new_password);
                const updates = {
                ...
                publicAccessToken = tokens_1.createPublicAccessToken(user.uid, req.ip, user.access_token, env.tokenSalt);
