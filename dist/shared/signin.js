    ...
    try {
        ...
        switch (credentials.method) {
            case 'access_token': {
                ...
                try {
                    tokenDetails = tokens_1.decodePublicAccessToken(credentials.access_token, env.tokenSalt);
        ...
        if (credentials.method !== 'access_token') {
            let hash = user.password_salt ? password_1.getPasswordHash(credentials.password, user.password_salt) : password_1.getOldPasswordHash(credentials.password);
        ...
        if (credentials.method !== 'access_token') {
            if (!user.password_salt) {
                ...
                let pwd = password_1.createPasswordHash(credentials.password);
