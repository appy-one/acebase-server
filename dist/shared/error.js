...
const sendError = (res, err) => {
    ...
    if (typeof err.code === 'string') {
        exports.sendBadRequestError(res, err); //res.status(400).send({ code: err.code, message: err.message }); // Bad Request
    }
    else {
        exports.sendUnexpectedError(res, err); // res.status(500).send({ code: 'unknown', message: 'server error', details: err.message }); // Internal server error
    }
};
