...
export class SpotifyAuthProvider extends OAuth2Provider {
    ...
    async getAccessToken(params: IOAuth2AuthCodeParams|IOAuth2RefreshTokenParams) {
        ...
        const result = await response.json() as ISpotifyAuthToken;
    ...
    async getClientAccessToken() {
        ...
        const result = await response.json() as ISpotifyClientAuthToken;
        if ((result as any).error) {
            throw new Error((result as any).error);
        }
        return result;
